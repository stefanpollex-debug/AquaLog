...
export function PhotoScanner({ onResult }: Props) {
  ...
  const analyse = async (file: File | null | undefined) => {
    if (!file) return;
...
Falls gar kein Teststreifen erkennbar: {"error":"Kein Teststreifen erkennbar"}`,
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      {/* Always-mounted hidden input — stays in DOM across all states so retake() can trigger it */}
      <input
        ref={inputRef}
        type="file"
        accept="image/*"
      ...
      {state === "loading" && (
        <div style={{ background: "#f0f9ff", borderRadius: 14, padding: 20, textAlign: "center" }}>
          {preview && <img src={preview} alt="Vorschau" style={{ width: "100%", maxHeight: 110, objectFit: "cover", borderRadius: 10, marginBottom: 12 }} />}
          <style>{`@keyframes bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}`}</style>
          <div style={{ display: "flex", justifyContent: "center", gap: 6, marginBottom: 8 }}>
            {[0, 1, 2].map((i) => (
              <div key={i} style={{ width: 10, height: 10, borderRadius: "50%", background: "#0ea5e9", animation: `bounce 1s ease ${i * 0.2}s infinite` }} />
            ))}
          </div>
      ...
      {state === "result" && aiResult && (
        <div style={{ background: "#f0fdf4", border: "1.5px solid #bbf7d0", borderRadius: 14, padding: 16 }}>
          {preview && <img src={preview} alt="Teststreifen" style={{ width: "100%", maxHeight: 100, objectFit: "cover", borderRadius: 10, marginBottom: 12 }} />}
