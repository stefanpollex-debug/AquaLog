...
function drawChartCanvas(entries: PoolEntry[], field: FieldKey, widthPx: number, heightPx: number): string {
  const scale  = 2;
...
export function exportPDF(entries: PoolEntry[], profile: PoolProfile): void {
  ...
  if (sorted.length >= 3) {
    ...
    (["cl", "ph", "temp"] as FieldKey[]).forEach(field => {
      const img = drawChartCanvas(sorted, field, 1092, 240); // 182mm × 40mm @ 150dpi
      doc.setFontSize(7.5);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(100, 116, 139);
